...
(${system.properties.name[0].toUpperCase()}${system.properties.name
    ...
    .toLowerCase()}) 
    checks your water as it leaves the system, but not after it goes through pipes to get to you.</p>
...
function getSystemHTML(website_blurb, system) {
  return `<h3 class="card-title">Where your water comes from</h3>
    ...
    .substr(1, system.properties.name.length)
    .toLowerCase()}</h4>
    <p class="card-text">Your treatment system keeps the most detailed information about your water. If it's not safe to drink, they will have advice about what to do. ${website_blurb} </p>`;
}
